...
class OpenGLRenderer : public IRenderer
{
	...
	public:
		...
		*/
		...
		const unsigned short MAX_TEX_LAYER = 3;		// Current maximum layers for OpenGL multitextures.
		...
		*/
		...
		/**	@brief: Generate a VBO.
			...
			@return: NONE (handle passed via reference technically returned).
		*/
		void VBO_Gen(GLuint& bufferHandle, GLsizei size, float data[]);
		...
		/**	@brief: Bind (Access) the VBO object.
			...
			@return: NONE.
		*/
		void VBO_Bind(GLuint bufferHandle);
		...
		/**	@brief: Fill a VBO with data (position, color, or UV). NOTE: CURRENTLY NON-FUNCTIONAL.
			...
			@return: NONE.
		*/
		void VBO_Fill(GLuint bufferHandle, float data[], unsigned int size, GLsizei offset);
		...
		*/
		...
		/**	@brief: Enable a vertex attribute.
			...
			@return: NONE.
		*/
		void enableVertexAttributes(unsigned int enable);
		...
		/**	@brief: Bind a buffer to a vertex attribute.
			...
			@return: NONE.
		*/
		void BindBuffer(unsigned int vertexAttribute, GLuint &VBO_handle);
		...
		/**	@brief: Disable a vertex attribute.
			...
			@return: NONE.
		*/
		void disableVertexAttributes(bool disableVertex, bool disableColor = false, bool disableUV = false);
		...
		/**	@brief: Draw the VBO.
			...
			@return: NONE.
		*/
		void DrawAsTriangles(unsigned int size);
		...
		*/
		GLuint CreateLinkToShader(GLuint ShaderProgram, const char* name);
		...
		/**	@brief: Link a texture object to a shader program.
			...
			@return: (GLuint) - Address value of the variable 'name' inside the shader program.
		*/
		GLuint LinkTexObjToShader(GLuint textureHandle, GLuint ShaderProgram, const char* name);
		...
		*/
		GLuint LinkDataToShaderParameter(GLuint VBO_Handle, GLuint ShaderProgram, const char* attributeName, GLfloat data[], GLuint valuesPerVertex);
		...
		*/
		void Tex_Gen(GLuint &textureHandle);
		...
		/**	@brief: Delete an OpenGL texture object.
			...
			@return: NONE.
		*/
		void Tex_Del(GLuint &textureHandle);
		...
		/**	@brief: Open the texture accessed by 'textureHandle'.
			...
			@return: NONE.
		*/
		void Tex_SetAsActive(GLuint &textureHandle);
		...
		/**	@brief: Load data into an OpenGL texture object.
			...
			@return: NONE.
		*/
		void Tex_2D(GLuint textureHandle, GLint mipMapLevel, GLsizei width, GLsizei height, const sf::Uint8* pixelArray);
		...
		/**	@brief: Generate texture mipmaps.
			...
			@return: NONE.
		*/
		void Tex_GenMipMaps(GLuint textureHandle);
		...
		/**	@brief: Set the mipmap behaviour of an OpenGL texture object.
			...
			@return: NONE.
		*/
		void Tex_SetMipMapBehaviour(GLuint textureHandle, GLuint behaviourtype);
		...
		/**	@brief: Set the texture wrapping behaviour to REPEATING.
			...
			@return: NONE.
		*/
		void Tex_SetWrapToRepeating(GLuint textureHandle);
		...
		/**	@brief: Set the texture wrapping behaviour to CLAMPING.
			...
			@return: NONE.
		*/
		void Tex_SetWrapToClamping(GLuint textureHandle);
		...
		/**	@brief: Set the minification / magnification filtering to LINEAR.
			...
			@return: NONE.
		*/
		void Tex_SetFilteringToLinear(GLuint textureHandle);
		...
		/**	@brief: Set the minification / magnification filtering to NEAREST.
			...
			@return: NONE.
		*/
		void Tex_SetFilteringToNearest(GLuint textureHandle);
